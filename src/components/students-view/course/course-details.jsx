...
const CourseDetails = () => {
    const { studentCourse, setStudentCourse, studentCourseId, setStudentCourseId } = useStudentContext();
                ...
                <>
                    <Card className="mb-4">
                        <CardBody>
                            ...
                            <CardText><strong>Duration:</strong> {studentCourse.duration} weeks</CardText>
                            <CardText><strong>Status:</strong> {studentCourse.status}</CardText>
                        </CardBody>
                    </Card>
                    
                    <h3 className="mt-4">Subjects and Materials</h3>
                    <ListGroup>
                                    ...
                                    <CardBody>
                                        ...
                                        <ListGroup>
                                            {subject.materials.map((material) => (
                                                <ListGroupItem key={material._id} className="d-flex justify-content-between align-items-center">
                                                    <div className="flex-grow-1">
                                                        <strong>{material.name}</strong> - {material.content_type}
                                                        <p className="material-description">{material.description}</p>
                                                    </div>
                                                    <div>
                                                        {material.content_type === 'PDF' && (
                                                            <Button color="primary" size="sm" href={material.content_url} target="_blank" rel="noopener noreferrer">
                                                                View PDF
                                                            </Button>
                                                        )}
